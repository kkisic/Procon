...
signed main(){
    ...
    int pbw = 1;
    for(int i = 1; i <= b + w; i++){
        int ans = (pb - pbw * powM(2, MOD - 2) % MOD + MOD) % MOD;
        cout << ans * powM(powM(2, MOD - 2), i - 1) % MOD << endl;

        pb = (2 * pb % MOD + MOD - c.comb(i - 1, b - 1)) % MOD;
        pbw = (2 * pbw % MOD + MOD - c.comb(i - 1, b - 1)) % MOD;
        pbw = (pbw + MOD - c.comb(i - 1, i - w)) % MOD;
    }
